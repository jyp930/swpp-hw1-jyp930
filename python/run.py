...
def main():
    ...
    common_names_dict = {}
    for each_parsed_malenames in parsed_malenames:
        common_names_dict[each_parsed_malenames[1]] = [each_parsed_malenames[0]]
    for each_parsed_femalenames in parsed_femalenames:
        if each_parsed_femalenames[1] in common_names_dict:
            common_names_dict[each_parsed_femalenames[1]].append(each_parsed_femalenames[0])

    for each_common_names_dict in common_names_dict:
        if len(common_names_dict[each_common_names_dict]) == 2: #공통 이름일 경우
            common_names.append(each_common_names_dict+": "+common_names_dict[each_common_names_dict][0]+", "+common_names_dict[each_common_names_dict][1])
