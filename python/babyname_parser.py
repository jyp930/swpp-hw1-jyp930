...
def check_filename_existence(func):
    ...
    def check_filename_existence_wrapper(*args):
        if not os.path.isfile(args[1]):
            raise BabynameFileNotFoundException("No such babyname file or directory: {filename}".format(filename = args[1]))
        else:
            func(*args)

    return check_filename_existence_wrapper
...
class BabynameParser:
    ...
    def parse(self, parsing_lambda):
        ...
        result = []
        for each_tuple in self.rank_to_names_tuples:
            result.append(parsing_lambda(each_tuple))
